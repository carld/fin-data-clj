...
(def ocr-source-url "http://www.rbnz.govt.nz/monetary-policy/official-cash-rate-decisions")

(defn fetch-page []
  (-> (client/get ocr-source-url) :body parse as-hickory))
...
(defn rates [r]
  (map (fn [row]
         (let [[date change rate] row]
           (hash-map
            :announced_on (-> (re-seq #"\d+\s+\w+\s+\d+" date) first parse-date c/to-timestamp)
            :link (let [link (-> (re-seq #"href=\"(.+)\"" date) first second)]
                    (cond
                      link  (str "http://www.rbnz.govt.nz" link)
                      :else nil))
            :rate (-> (re-seq #"\d+\.\d+" rate) first read-string))))
       (rest r) ; ignore the first header row
       ))
